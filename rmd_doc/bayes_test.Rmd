...
A/B testing is just marketing jargon for a randomized controlled trial. The test is conducted on a subset of users in order to determine if a change in service (often user experience) will have a positive impact on the business before rolling out that change to all the users.

Typically, A/B testing involves one group of people being served the existing content (control group) while another group being served different content (treatment group) and, through a measurable indicator, the business wants to determine if there is a difference in reaction between the two groups. If we compare the two groups and find that difference in the indicator is substantially large then we can argue that the different content drove that change. Conversely, if the change was minimal then you may be hesitant to conclude that the different content resulted any change in behavior at all. In that situation perhaps the content needs to be redesigned (and retested).

Most A/B testing approaches used in practice typcially rely on Frequentist hypothesis testing methods. Not only are the results of these methods difficult to interpret, but they can also be misleading. Terms such as "p-values" and "confidence intervals" are often misinterpreted as probabilities directly related to the parameter of interest (e.g. difference in means between two groups). Disconcertingly, p-values are often used as cutoffs for a business decision. In other words, reaching a statistically signficant result is often sufficient to convince a business to move forward with a particular decision. We argue that these descisions should not be reductively derived from arbitrary cutoffs (e.g. a p-value of less than 0.05). Instead they should be determined by domain-specific experts who understand the industry, while statisticians should provide interpretable results that can help these experts make more informed decisions. This case study provides a way for domain-specific experts to apply their knowledge to the statistical inference process through priors. Additionally, they can quantify the risk they're willing to take probabilistically, which can be incorporated into the inference.
...
At a high-level, we stress that Frequentist methods focus on the distribution of the test statsitic as opposed to the quantity of interest (i.e. predictions or parameters). In such methods inference is done by understanding how the observed test statistic relates to the distribution of the test statistic under the null hypothesis. Alternatively, the Bayesian methods proposed here allow the researcher to perform inference directly on the quantity of interest (in this case predicted data), which is more useful in the context of A/B testing.

## Continuous Data

_This example is analogous to the two sample t-test (specifically Welch's t-test) where the researcher is interested in testing if there is a noticeable difference between the means of two different samples._
...
With regard to priors, we have applied $\mathcal{N}(3,1)$ distributions on both intercepts. The reasoning behind this is that the empirical mean of the control group is approximately $3$. By applying this prior to the treatment group we are quantifying our belief that the treatment group is not substantially different to the control group. (Which is a reasonable belief to have since the whole purpose of the experiment is to evaluate whether the two group are actually different.) This allows us to be more conservative when we do our inference. If we end up concluding that the two groups are different, we can say that the difference in behavior was so strong that it overcame our belief that the two groups are identical.
...
After computing the $90\%$ credible interval for both groups we find an overlap proportion of approximately $0.2$. Thus, given that we are $90\%$ sure about our posterior predictions for the two groups, we are about $80\%$ confident that the two groups are in fact different. Going back to the business context, we can conclude that we are $80\%$ confident that reducing ads increases daily streaming consumption given our acceptable risk of being $10\%$ wrong about daily streaming consumption.

In the plot below we show how the overlap proportion will vary as the credibel interval varies. To put it differently, it shows how the probabilistic difference between groups varies as risk varies. Notice that the more risk we take when defining our credible interval (i.e. the closer the credible interval is to 0) the lower the overlap proportion, and consequentially the more confident we are about the difference between the two groups.
...
_This example is analogous to Fisher's exact test where the researcher is interested in testing differences in proportions (particularly in the form of a contingency table)._
...
Below we show how the overlap proportion varies based on the amount of risk we're willing to take when we define our credible intervals.
...
Note, this example involved a really small data set (only one observation for each group). But the same model can easily be extended to many observations within each group. Also, just as we described in the continous example, we can define a more comprehensive model for the outcome if we had predictors. 

## Benefits of Bayesian Methods

**Interpretation of probability**

Below are some of the advantages of taking a Bayesian inference approach to A/B testing:
...
4. We can construct the following conclusion: given there is a $(1-c)100$ percent chance that we are wrong about the predictions, there is a $(1-p)100$ percent chance that the two groups are different.

The Bayesian methods outlined in this case study focused on modeling the data generation process and performing inference on the posterior predictive distribution of two groups. We did not need to worry about computing test statistics and determining the distribution of these statistics under the null hypothesis. Nor did we need to calculate p-values to figure out whether the groups involved in A/B testing are different. Instead we performed inference directly on the posterior predictions. By constructing credible intervals and computing the overlap of these intervals we are able to probabilistically convey how sure we are about the difference between the two groups. This enables statisticians to quantify business risk and allows them to communicate uncertainty unambiguously to decision makers, allowing more informed decisions to be made.
