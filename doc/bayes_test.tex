...
\noindent This model can be defined in Stan as,
...
\noindent Fitting this model to the data sample in rstan gives samples that define the marginal posterior distributions for both $\mu$ and $\sigma$, along with the posterior predictive distribution for $x$. These distributions are illustrated in the figure below with the mean highlighted in red. \\
...
\noindent We are still interested in testing if the mean is different from the hypothesized value, 4.5. To do this we can compute sample quantiles using probabilities defined by the researcher. These \emph{uncertainty intervals} enable us to communicate the probability that the estimated mean is between two values conditional on the data and prior information. Looking at the figure below, which illustrates the 50\% and 90\% credible intervals for the location parameter, we identify that 90\% of the marginal posterior is between 3.3 and 5.7. It is at the discretion of the researcher to conclude whether this interval is satisfactory to conclude that the estimated mean is not different from 4.5. If the 90\% uncertainty interval excluded 4.5 then we could say that there is a 90\% chance that the mean estimated from the sample is different from 4.5. \\
