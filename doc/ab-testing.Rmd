...
1. Based on prior knowledge (past data and/or domain specific experts) we believe that users spend around three hours per day on the service (regarless of what our random sample says).
...
After computing the $90\%$ prediction interval for both groups we find an overlap proportion of approximately $0.25$. Thus, given that we are $90\%$ sure about our posterior predictions for the two groups, we are about $75\%$ sure that the two groups are in fact different. Going back to the business context, we can conclude that we are $75\%$ sure that reducing ads increases daily streaming consumption given our acceptable risk of being $10\%$ wrong about daily streaming consumption. <!-- Given our modeling assumptions, etc. -->
...
$$
where $\mu$ is the parameter for each group, $group$ is a binary variable indicating group memebership, $y$ is the number of users that purchased the product and $N$ is the total number of users in each group. Below we fit this model to the data.
...
Here we show how to use hierachical (or multilevel) models as an alternative modeling approach when performing A/B tests. Using the data in the continuous example we want to build a model where we account for group-level intercepts while allowing information to be shared among the groups.
