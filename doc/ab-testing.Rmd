...
The data collected might look something like the following below. Each observation is a user's daily streaming consumption in hours. We also have an additional variable `hc` which defines whether a user is predisposed to being a _high consumer_ of streaming content.

```{r}
group <- c(rep(1,10), rep(2,12))
group <- factor(c(rep("A",10), rep("B",12)))
N <- length(group)
hc <- sample(c(0,1), N, replace = TRUE)
effect <- c(3,5)

lp <- effect[group] + 1.5*hc
y <- rnorm(N, lp, 0.5)
...
$$
\begin{align*}
y \sim &\mathcal{N}(\mu_A + \mu_B + \beta \cdot \mbox{hc}, \sigma) \\
& \mbox{priors on } \mu_A, \mu_B, \beta, \sigma \\
...
With regard to priors, we have applied $\mathcal{N}(3,1)$ distributions on both group effects. The reasoning behind this is that the empirical mean of the control group (group A) is approximately $3$. By applying this prior to the treatment group (group B) we are quantifying our belief that the treatment group is not substantially different to the control group, which is our null hypothesis. (This is a reasonable belief to have since the whole purpose of the experiment is to evaluate whether the two groups are actually different.) This can be interpreted as incorporating the belief underlying our null hypothesis into the prior. More importantly, this approach allows us to be more conservative when we do our inference. If we end up concluding that the two groups are different, we can say that the difference in behavior was so strong that it overcame our prior belief that the two groups are identical. Our final model is,
...
```{r results='hide'}
fit_group <- stan_glm(y ~ 0 + group + hc,
                      data = experiment,
                      family = gaussian(link="identity"),
                      prior = normal(c(3,3,0), 1, autoscale = FALSE), seed = 123)
```

Recall that Stan uses a sampling algorithm to estimate the marginal posterior distribution of the parameters which means that we have samples instead of point estimates for the intercept values. The medians for each group after controlling for the dependent variable are,
...
pp_a <- posterior_predict(fit_group, newdata = data.frame(group = factor("A"), hc = experiment$hc))
pp_b <- posterior_predict(fit_group, newdata = data.frame(group = factor("B"), hc = experiment$hc))
...
Since we modeled the outcome using a predictor we can vary the predictor as well as group membership for an observation for more detailed inference. Below we plot the credible intervals for each group and high consumer variable combination. This allows to us compare the difference in average streaming hours among the two groups for those individuals that were categorized as high/low consumers.

```{r fig.align='center', fig.height=10, fig.width=10}
pp_a0 <- posterior_predict(fit_group, newdata = data.frame(group = factor("A"), hc = 0))
pp_b0 <- posterior_predict(fit_group, newdata = data.frame(group = factor("B"), hc = 0))
pp_a1 <- posterior_predict(fit_group, newdata = data.frame(group = factor("A"), hc = 1))
pp_b1 <- posterior_predict(fit_group, newdata = data.frame(group = factor("B"), hc = 1))
...
$$
y_g \sim \mbox{Bin}(\mbox{logit}^{-1}(\mu_g), N_g)\\
$$
Below we fit this model to the data.
